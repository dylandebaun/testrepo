...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        data = spacex_df_success['Launch Site'].value_counts().rename_axis('Launch Site').reset_index(name='successful launches')
        fig = px.pie(data, values='successful launches', 
        names='Launch Site', 
        title='Total Success Launches By Site')
        return fig
    else:
        filtered = filtered_df[filtered_df['Launch Site'] == entered_site]
        data = filtered['class'].value_counts().rename_axis('class').reset_index(name='counts')
...
def get_scatter_plot(entered_site,payload):
    ...
    if entered_site == 'ALL':
        ...
        filtered = filtered_df[filtered_df['Launch Site'] == entered_site]
        fig = px.scatter(filtered, x="Payload Mass (kg)", y="class", color="Booster Version Category",title="Correlation between Payload and Success for Site")
        return fig
